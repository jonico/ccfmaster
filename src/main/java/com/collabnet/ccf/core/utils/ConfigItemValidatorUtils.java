...
public abstract class ConfigItemValidatorUtils {
    ...
    public static void validateValue(List<CCFCoreProperty> configProperties,
            Errors errors, String errorElementName) {
        for (int i = 0; i < configProperties.size(); i++) {
            ...
            String conditionalRegex = configProperties.get(i)
                    .getConditionalRegex();
            if (StringUtils.isEmpty(value)) {
                errors.rejectValue(errorElementName + "[" + i + "].value",
                ...
                errors.rejectValue(errorElementName + "[" + i + "].value",
                        PARTICIPANT_SHOULD_MATCH_CONDITIONAL_REGEX);
            }
        }
    }
