...
public class IafMenu extends TagSupport {
	...
	private static final Logger log = LoggerFactory.getLogger(IafMenu.class);
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			throw new JspException("Error getting data from TeamForge: " + e.getMessage(), e);
		}
	}
	...
	private String getBanner(IafUserDetails user, HttpServletRequest request) throws ClientProtocolException, IOException, URISyntaxException {
		...
		URI uri = URIUtils.createURI(
				tfUri.getScheme(), tfUri.getHost(), tfUri.getPort(),
		...
		httpget.setHeaders(new Header[] {
				...
				new BasicHeader("Accept-Language", request.getLocale().toString()),
				new BasicHeader("User-Agent", "Mozilla/5.0 (Windows; U; WinNT4.0; en-US; rv:1.7.5) BloodRedSun/0.5"),
		});
		...
		/*
		 * the following hacky string replacements and additions are adopted from
		...
		for (String line : CharStreams.readLines(isr)) {
			buf.append(
					line
					.replaceAll("window.location=", "parent.window.location=")
					.replaceAll("<a class=\"yuimenubaritemlabel\"", "<a class=\"yuimenubaritemlabel\" target=\"_top\"")
					.replaceAll("<a class=\"yuimensomeuitemlabel\"", "<a class=\"yuimensomeuitemlabel\" target=\"_top\"")
					.replaceAll("<a class=\"yuimenuitemlabel\"", "<a class=\"yuimenuitemlabel\" target=\"_top\"")
//					.replaceAll("window.location='/sf", "window.location='" + serverUrl + "/sf")
//					.replaceAll("/sf/global/do/about", serverUrl + "/sf/global/do/about")
	...
	private String cssInclude(String serverUrl) {
		return String.format("<link rel=\"stylesheet\" type=\"text/css\" href=\"%s\" />%n", cssUrl(serverUrl));
	}
	...
	private String jsIncludes(String serverUrl) {
		...
		String[] scriptFiles = {
				...
				"/js/yui/container_core-min.js",
				"/js/yui/menu-min.js"
		};
		for (String scriptFile : scriptFiles) {
			htmlString.append(String.format("<script src=\"%s%s\" type=\"text/javascript\"></script>%n", baseUrl, scriptFile));
		}
	...
	private String makeComponentLinkUrl(String baseUrl, String projectPath, String plugId) {
		String res = new StringBuilder(baseUrl)
			.append("/pluggable/do/viewPluggableComponent/")
			.append(projectPath)
			.append("/")
			.append(plugId)
			.toString();
