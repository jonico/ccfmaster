...
public class CreateRMDHelper {
    ...
    public void buildRepositoryMappingDir(Model model, RMDModel rmdmodel,
            ConflictResolutionPolicy conflictPolicy, Directions directions,
            String templateName, ExternalApp extApp) {
        String teamForgeRepositoryId = getTeamForgeRepoId(rmdmodel);
        String participantRepositoryId = getParticipantRepoId(rmdmodel);
        RepositoryMapping repositoryMapping = getRespositoryMapping(extApp,
                teamForgeRepositoryId, participantRepositoryId);
        if (isValidateRMDAgainstExternalApp()) {
            validateRepositoryMapping(extApp, repositoryMapping);
        }
        RepositoryMappingDirection repoMappingDirection = getRepositoryMappingDirection(
                directions, repositoryMapping, conflictPolicy);
        FieldMapping fieldMapping = getFieldMapping(templateName, directions,
    ...
    public void createAndPersistRMD(RMDModel rmdmodel, Model model,
            String direction, ExternalApp extApp) {
        ...
        if (!rmdmodel.getForwardConflictPolicies().isEmpty()) {
            forwardConflictPolicy = ConflictResolutionPolicy.valueOf(rmdmodel
                    .getForwardConflictPolicies());
        }
        if (!rmdmodel.getReversedConflictPolicies().isEmpty()) {
            reverseConflictPolicy = ConflictResolutionPolicy.valueOf(rmdmodel
                    .getReversedConflictPolicies());
        }
        if ("FORWARD".equalsIgnoreCase(direction)) {
            buildRepositoryMappingDir(model, rmdmodel, forwardConflictPolicy,
                    Directions.FORWARD,
                    rmdmodel.getForwardFieldMappingTemplateName(), extApp);
        } else if ("REVERSE".equalsIgnoreCase(direction)) {
            buildRepositoryMappingDir(model, rmdmodel, reverseConflictPolicy,
                    Directions.REVERSE,
                    rmdmodel.getReverseFieldMappingTemplateName(), extApp);
        } else {
            buildRepositoryMappingDir(model, rmdmodel, forwardConflictPolicy,
                    Directions.FORWARD,
                    rmdmodel.getForwardFieldMappingTemplateName(), extApp);
    ...
    public String getAllTrackerInfo(String projectId) {
        ...
        try {
            teamForgeTracker = TeamForgeMetadataHelper
                    .getAllTrackersOfProject(projectId);
    ...
    public List<String> getFieldMappingTemplateNames(Directions direction) {
        ...
        List<FieldMappingLandscapeTemplate> fieldMappingLandscapeTemplates = FieldMappingLandscapeTemplate
                .findFieldMappingLandscapeTemplatesByDirection(direction)
                .getResultList();
    ...
    public void populateConfigMaps(RMDModel rmdmodel) {
        List<ParticipantConfig> participantConfig = ParticipantConfig
                .findAllParticipantConfigs();
        List<LandscapeConfig> landscapeConfig = LandscapeConfig
                .findAllLandscapeConfigs();
    ...
    public void setValidateRMDAgainstExternalApp(
            boolean validateRMDAgainstExternalApp) {
    ...
    private List<FieldMappingRule> cloneFieldMappingRules(
    ...
    private List<FieldMappingValueMap> cloneFieldMappingValueMap(
    ...
    private List<FieldMappingValueMapEntry> cloneFieldMappingValueMapEntry(
    ...
    private FieldMapping getFieldMapping(String templateName,
        ...
        FieldMappingLandscapeTemplate fieldMappingLandscapeTemplate = FieldMappingLandscapeTemplate
                .findFieldMappingLandscapeTemplatesByParentAndNameAndDirection(
                        landscape, templateName, direction).getSingleResult();
        List<FieldMapping> fieldMappingList = FieldMapping
                .findFieldMappingsByNameAndParentAndScope(templateName,
                        repoMappingDirection,
    ...
    private RepositoryMappingDirection getRepositoryMappingDirection(
            Directions direction, RepositoryMapping repositoryMapping,
            ConflictResolutionPolicy conflictPolicy) {
        List<RepositoryMappingDirection> repositoryMappingDirectionList = RepositoryMappingDirection
                .findRepositoryMappingDirectionsByRepositoryMappingAndDirection(
                        repositoryMapping, direction).getResultList();
    ...
    private RepositoryMapping getRespositoryMapping(ExternalApp externalApp,
            String teamForgeId, String participantID) {
        List<RepositoryMapping> repositoryMappingList = RepositoryMapping
                .findRepositoryMappingsByExternalAppAndParticipantRepositoryIdAndTeamForgeRepositoryId(
                        externalApp, participantID, teamForgeId)
                .getResultList();
    ...
    private String getTeamForgeRepoId(RMDModel rmdmodel) {
        ...
        String teamForgeRepositoryId = String.format("%s-%s-%s", projectId,
                trackerId, rmdmodel.getTeamForgeMappingType());
        if (rmdmodel.getTeamForgeMappingType().equalsIgnoreCase("tracker")) {
            teamForgeRepositoryId = trackerId;
        } else if (rmdmodel.getTeamForgeMappingType().equalsIgnoreCase(
                PLANNINGFOLDERS)) {
            teamForgeRepositoryId = String.format("%s-%s", projectId,
                    PLANNINGFOLDERS);
        } else if (rmdmodel.getTeamForgeMappingType().equalsIgnoreCase(
                "MetaData")) {
            teamForgeRepositoryId = String.format("%s-%s", trackerId,
                    rmdmodel.getTeamForgeMappingType());
        }
    ...
    private static Map<String, String> buildConfigMap(
            List<? extends ConfigItem> configList) {
