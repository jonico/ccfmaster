...
public class IndexController {
	...
	/**
	 * Redirect the user to the appropriate view. The view is determined as follows:
	 * <ul>
	 ...
	 * <li>else throw a {@link BadCredentialsException}, which causes Spring security to redirect to the login window.</li>
	 * </ul> 
	 * @throws BadCredentialsException if user not authorized sufficiently.
...
//	@PreAuthorize("isAuthenticated() and hasRole('ROLE_TF_USER')")
	...
	public String index() {
		...
		else if (principal instanceof TFUserDetails && auth.getAuthorities().contains(TFUserDetails.SUPER_USER))
			return "redirect:" + UIPathConstants.CREATELANDSCAPE_CCFMASTER;
		else
			throw new AccessDeniedException("Not authenticated as a TeamForge Site Admin or IAF user with sufficient permissions.");
	}
