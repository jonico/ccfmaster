...
public class LandscapeStatusController extends AbstractLandscapeController{
	...
	*/  
    ...
    public String displayStatusTFtoParticipant(Model model, HttpServletRequest request) {
	...
	public String startCCFCoreStatus(
		...
		mergeCCFCoreStatus(model,ccfCoreStatusModel,request,directions,ExecutedCommand.START);
		} catch(CoreConfigurationException coreConfigurationException) {
			log.debug("Error starting core: " + coreConfigurationException.getMessage(), coreConfigurationException);
			FlashMap.setErrorMessage(ControllerConstants.CCF_CORE_STATUS_MESSAGE, coreConfigurationException.getMessage());
		}
		model.asMap().clear();
		if (paramdirection.equals(ControllerConstants.FORWARD)) {
			return  "redirect:/" +UIPathConstants.LANDSCAPESETTINGS_DISPLAYTFTOPARTICIPANTSTATUS;
		}else{
			return  "redirect:/" +UIPathConstants.LANDSCAPESETTINGS_DISPLAYPARTICIPANTTOTFSTATUS;
		}
	}
	...
	public String stopCCFCoreStatus(
		...
		try{
		Directions directions = ControllerConstants.FORWARD.equals(paramdirection) ? Directions.FORWARD : Directions.REVERSE;
		mergeCCFCoreStatus(model,ccfCoreStatusModel,request,directions,ExecutedCommand.STOP);
		} 
		catch(CoreConfigurationException coreConfigurationException){
			log.debug("Error stopping core: " + coreConfigurationException.getMessage(), coreConfigurationException);
			FlashMap.setErrorMessage(ControllerConstants.CCF_CORE_STATUS_MESSAGE, coreConfigurationException.getMessage());
		}
		model.asMap().clear();
		if (paramdirection.equals(ControllerConstants.FORWARD)) {
	...
	*/  
    ...
    public String displayStatusParticipanttoTF(Model model, HttpServletRequest request) {
	...
	*/  
	...
	public static void restartCCFCoreStatus(Model model) {
		try{
			List<CcfCoreStatus> coreStatus= CcfCoreStatus.findAllCcfCoreStatuses();
