...
public class ApiCcfCoreStatusController extends AbstractApiController<CcfCoreStatus>{
	...
	private static final Logger log = LoggerFactory.getLogger(ApiCcfCoreStatusController.class);
		
	@Override	
	public @ResponseBody CcfCoreStatus create(@RequestBody CcfCoreStatus requestBody, HttpServletResponse response) {
		throw new BadRequestException("create not supported");
	}
	
	@Override	
	public @ResponseBody CcfCoreStatusList list() {
		return new CcfCoreStatusList(CcfCoreStatus.findAllCcfCoreStatuses());
	}
	
	
	@Override	
	public @ResponseBody CcfCoreStatus show(@PathVariable("id") CcfCoreStatus id) {
		return super.show(id);
	}
	
	@Override	
	public void update(@PathVariable("id") Long id, @RequestBody CcfCoreStatus requestBody, HttpServletResponse response) {
	...
	private void validateRequestBody(Long id, CcfCoreStatus requestBody) {
		if (id == null || !id.equals(requestBody.getId())) {
			throw new BadRequestException(String.format("id (%s) != ccfCoreStatus.id (%s)", id, requestBody.getId()));
		}
	}
