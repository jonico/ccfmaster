...
public class LinkIdApiRepositoryMappingDirectionController extends AbstractApiLinkIdController<RepositoryMappingDirection> {
    ...
    RepositoryMappingDirection create(
        ...
        setLocationHeader(
                response,
                Paths.LINKID_REPOSITORY_MAPPING_DIRECTION.replace("{linkId}",
                        ea.getLinkId()) + "/" + requestBody.getId());
        return requestBody;
    }
    ...
    public void delete(
        ...
        RepositoryMappingDirection rm = RepositoryMappingDirection
    ...
    public @ResponseBody
    RepositoryMappingDirectionList repositoryMappingDirectionDirectionScope(
            @PathVariable("direction") Directions direction,
            @ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea) {
        return new RepositoryMappingDirectionList(RepositoryMappingDirection
                .findRepositoryMappingDirectionsByExternalAppAndDirection(ea,
    ...
    public @ResponseBody
    FieldMappingList showFieldMappings(
            @ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea,
            @PathVariable("id") Long id) {
        RepositoryMappingDirection rmd = RepositoryMappingDirection
                .findRepositoryMappingDirection(id);
        validateRepositoryMappingDirection(rmd);
        return new FieldMappingList(FieldMapping.findFieldMappingsByParent(rmd)
                .getResultList());
    }
    ...
    private void validateRequestBody(Long id,
        ...
        RepositoryMappingDirection original = RepositoryMappingDirection
                .findRepositoryMappingDirection(id);
        if (!original.getRepositoryMapping().getId()
                .equals(requestBody.getRepositoryMapping().getId())) {
            throw new AccessDeniedException(
