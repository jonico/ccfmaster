...
public class LinkIdApiIdentityMappingController extends AbstractApiLinkIdController<IdentityMapping> {
	...
	@Override
	public @ResponseBody IdentityMapping create(@ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea, @RequestBody IdentityMapping requestBody, HttpServletResponse response) {
		validateIdentityMapping(requestBody);
		requestBody.persist();
		setLocationHeader(response, Paths.LINKID_IDENTITY_MAPPING.replace("{linkId}", ea.getLinkId()) + "/" + requestBody.getId());
		return requestBody;
	}

	@Override
	public @ResponseBody IdentityMappingList list(@ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea) {
		return new IdentityMappingList(IdentityMapping.findIdentityMappingsByExternalApp(ea).getResultList());
	}

	@Override
	public @ResponseBody IdentityMapping show(@ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea, @PathVariable("id") Long id) {
	...
	@Override
	public void update(@ModelAttribute(EXTERNAL_APP_MODELATTRIBUTE_NAME) ExternalApp ea, @PathVariable("id") Long id, @RequestBody IdentityMapping requestBody, HttpServletResponse response) {
		...
		original.setSourceArtifactVersion(requestBody.getSourceArtifactVersion());
		original.setSourceLastModificationTime(requestBody.getSourceLastModificationTime());
		original.setTargetLastModificationTime(requestBody.getTargetLastModificationTime());
		original.setTargetArtifactVersion(requestBody.getTargetArtifactVersion());
		original.merge();
	}
	...
	private void validateRequestBody(Long id, IdentityMapping requestBody) {
		...
		IdentityMapping original = IdentityMapping.findIdentityMapping(id);
		if (!original.getRepositoryMapping().equals(requestBody.getRepositoryMapping())) {
