...
public class LandscapeTFSettingsController extends AbstractLandscapeController {
    ...
    private static final Logger log              = LoggerFactory
                                                         .getLogger(LandscapeTFSettingsController.class);
    ...
    @RequestMapping(value = "/"
            + UIPathConstants.LANDSCAPESETTINGS_DISPLAYTFSETTINGS, method = RequestMethod.GET)
    ...
    @Transactional
    public String saveTFSettings(
            @ModelAttribute("tfsettingsmodel") @Valid TFSettingsModel tfSettingsModel,
        ...
        if (bindingResult.hasErrors()) {
            ...
            try {
                tfSettingsHelper.updateTFSettings(tfSettingsModel, model,
                        request);
                ...
                log.debug(
                        "Error saving TF settings: " + exception.getMessage(),
                        exception);
                FlashMap.setErrorMessage(
                        ControllerConstants.TF_SAVE_FAIL_MESSAGE,
                        exception.getMessage());
            }
            model.asMap().clear();
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_DISPLAYTFSETTINGS;
        }
    }
    ...
    String testTeamForgeConnection(@RequestParam("username") String username,
        ...
        try {
            ...
            log.debug(
                    "Error testing TeamFOrge connection: "
