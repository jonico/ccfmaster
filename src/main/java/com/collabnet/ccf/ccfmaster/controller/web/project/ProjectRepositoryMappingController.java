...
public class ProjectRepositoryMappingController extends AbstractProjectController {
    ...
    public static final String  PROJECT_REPOSITORY_MAPPING_PATH = "/"
                                                                        + PROJECT_REPOSITORY_MAPPING_NAME;
    ...
    public String delete(
        ...
        Iterable<RepositoryMappingDirection> validRmds = filter(
                Arrays.asList(rmds), isValidRmd(ea, direction));
        try {
            ...
            FlashMap.setErrorMessage(
                    ControllerConstants.RMDDELETEFAILUREMESSAGE, e.getMessage());
        }
    ...
    public String list(
        ...
        List<RepositoryMappingDirection> rmds = paginate(
                RepositoryMappingDirection.findRepositoryMappingDirectionsByExternalAppAndDirection(
                        ea, direction),
                RepositoryMappingDirection
                        .countRepositoryMappingDirectionsByExternalAppAndDirection(
                                ea, direction), page, size, model)
                .getResultList();
        List<RepositoryMappingsModel> rmmList = LandscapeRepositoryMappingsController
                .makeRepositoryMappingsModel(rmds, tfUrl);
    ...
    private String redirectUrl(Directions direction) {
        return String.format("redirect:%s?%s=%s",
                PROJECT_REPOSITORY_MAPPING_PATH, DIRECTION_REQUEST_PARAM,
                direction);
    }
    ...
    private static Predicate<RepositoryMappingDirection> isValidRmd(
            final ExternalApp ea, final Directions direction) {
    ...
    private static void setStatusForRmds(
            Iterable<RepositoryMappingDirection> validRmds,
