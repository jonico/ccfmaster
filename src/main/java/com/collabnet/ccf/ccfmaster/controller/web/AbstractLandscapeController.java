...
public abstract class AbstractLandscapeController {
    ...
    /**
     * Paginate the {@code query}. Set first result and number of results to
     * return according to {@code page} and {@code pageSize}.
     * 
     * As a side effect, sets the model attribute "maxPages" to the number of
     * pages of size {@code pageSize}.
     * 
     * @param query
     * @param entityCount
     * @param page
     ...
     * @param model
     */
    protected static <T> TypedQuery<T> paginate(TypedQuery<T> query,
            long entityCount, Integer page, Integer pageSize, Model model) {
     ...
     * @return the modified query, so fluent api works. Identical to
     *         {@code query}.
     */
    ...
    private static int normalizePageNumber(Integer page) {
        final int pageNo = (page == null || page < 1) ? 0
                : (page.intValue() - 1);
        return pageNo;
    }

    /**
     * @return valid page size, defaults to
     *         {@link ControllerConstants#PAGINATION_SIZE}.
     */
    private static int normalizePageSize(Integer pageSize) {
        final int sizeNo = (pageSize == null || pageSize < 1) ? ControllerConstants.PAGINATION_SIZE
                : pageSize.intValue();
        return sizeNo;
    }
