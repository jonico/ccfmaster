...
public class LandscapeRepositoryMappingsController extends AbstractLandscapeController {
    ...
    public String deleteSynchronization(
        ...
        try {
            for (String rmId : items) {
                RepositoryMapping repositoryMapping = RepositoryMapping
                        .findRepositoryMapping(Long.valueOf(rmId));
                RepositoryMappingDirection repositoryMappingDirection = RepositoryMappingDirection
                        .findRepositoryMappingDirectionsByRepositoryMappingAndDirection(
                                repositoryMapping, directions)
                        .getSingleResult();
            ...
            FlashMap.setErrorMessage(
                    ControllerConstants.RMDDELETEFAILUREMESSAGE,
        ...
        if (paramdirection.equals(FORWARD)) {
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_DISPLAYREPOSITORYMAPPINGTFTOPART;
        } else {
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_DISPLAYREPOSITORYMAPPINGPARTTOTF;
        }
    }

    /**
     * Controller method to display repository mapping directions for
     * Participant to TF
     * 
     */
    @RequestMapping(value = "/"
            + UIPathConstants.LANDSCAPESETTINGS_DISPLAYREPOSITORYMAPPINGPARTTOTF, method = RequestMethod.GET)
    ...
    /**
     * Controller method to display repository mapping directions for TF to
     * Participant
     * 
     */
    @RequestMapping(value = "/"
            + UIPathConstants.LANDSCAPESETTINGS_DISPLAYREPOSITORYMAPPINGTFTOPART, method = RequestMethod.GET)
    ...
    public String pauseSynchronization(
            @RequestParam(ControllerConstants.DIRECTION) String paramdirection,
            Model model, HttpServletRequest request, HttpSession session) {
        setStatusForRMDs(RepositoryMappingDirectionStatus.PAUSED,
                paramdirection, model, request);
        model.asMap().clear();
    ...
    public String resumeSynchronization(
            @RequestParam(ControllerConstants.DIRECTION) String paramdirection,
            Model model, HttpServletRequest request, HttpSession session) {
        setStatusForRMDs(RepositoryMappingDirectionStatus.RUNNING,
                paramdirection, model, request);
        model.asMap().clear();
        Directions directions = ControllerConstants.FORWARD
                .equals(paramdirection) ? Directions.FORWARD
     ...
     * @param model
     */
    private void doList(Directions direction, Integer page, Integer size,
        ...
        List<RepositoryMappingDirection> rmds = paginate(
                ...
                RepositoryMappingDirection
                        .countRepositoryMappingDirectionsByDirection(direction),
    ...
    private void populateModel(Model model,
            List<RepositoryMappingDirection> rmds) {
        String tfUrl = ccfRuntimePropertyHolder.getTfUrl();
        List<RepositoryMappingsModel> rmmList = makeRepositoryMappingsModel(
                rmds, tfUrl);
    ...
    public static List<RepositoryMappingsModel> makeRepositoryMappingsModel(
            ...
            String tfUrl) {
        List<RepositoryMappingsModel> rmmList = new ArrayList<RepositoryMappingsModel>(
                repositoryMappingDirectionList.size());
        for (RepositoryMappingDirection rmd : repositoryMappingDirectionList) {
            ...
            rmm.setHospitalCount(HospitalEntry
                    .countHospitalEntrysByRepositoryMappingDirection(rmd));
            rmm.setIdentityMappingCount(IdentityMapping
                    .countIdentityMappingsByRepositoryMapping(rmd
                            .getRepositoryMapping()));
            Directions dir = rmd.getDirection();
            String tfRepoId = (dir == Directions.FORWARD) ? rmd
                    .getSourceRepositoryId() : rmd.getTargetRepositoryId();
            RepositoryDetail repositoryDetail = RepositoryConnections
                    .detailsForRepository(tfRepoId);
            ArtifactDetail artifactDetail = RepositoryConnections
                    .detailsForArtifact(rmd.getLastSourceArtifactId());
    ...
    public static void setStatusForRMDs(
            RepositoryMappingDirectionStatus status, String paramdirection,
        ...
        try {
            ...
            String messageCode = (status == RepositoryMappingDirectionStatus.PAUSED) ? ControllerConstants.RMDPAUSESYNCSUCCESSMESSAGE
                    : ControllerConstants.RMDRESUMESYNCSUCCESSMESSAGE;
            FlashMap.setSuccessMessage(messageCode);
        } catch (Exception exception) {
            String messageCode = (status == RepositoryMappingDirectionStatus.PAUSED) ? ControllerConstants.RMDPAUSESYNCFAILUREMESSAGE
                    : ControllerConstants.RMDRESUMESYNCFAILUREMESSAGE;
    ...
    private static List<RepositoryMappingDirection> setStatusForRMDs(
        ...
        for (String rmId : rmIds) {
            ...
            RepositoryMappingDirection repositoryMappingDirection = RepositoryMappingDirection
                    .findRepositoryMappingDirectionsByRepositoryMappingAndDirection(
                            repositoryMapping, direction).getSingleResult();
