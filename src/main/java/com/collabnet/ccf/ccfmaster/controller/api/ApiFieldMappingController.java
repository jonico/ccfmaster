...
public class ApiFieldMappingController extends AbstractApiController<FieldMapping> {
	...
	private static final Logger log = LoggerFactory.getLogger(ApiFieldMappingController.class);

	@Override
	public @ResponseBody FieldMapping create(@RequestBody FieldMapping requestBody, HttpServletResponse response) {
		requestBody.persist();
		setLocationHeader(response, Paths.FIELD_MAPPING + "/" + requestBody.getId());
		return requestBody;
	}
	
	@Override
	public @ResponseBody FieldMappingList list() {
		return new FieldMappingList(FieldMapping.findAllFieldMappings());
	}
	
	@Override
	public @ResponseBody FieldMapping show(@PathVariable("id") FieldMapping id) {
		return super.show(id);
	}
	
	@Override
	public void update(@PathVariable("id") Long id, @RequestBody FieldMapping requestBody, HttpServletResponse response) {
