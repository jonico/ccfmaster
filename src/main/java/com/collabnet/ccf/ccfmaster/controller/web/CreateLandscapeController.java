...
public class CreateLandscapeController {
    ...
    private static final Logger                log                                = LoggerFactory
                                                                                          .getLogger(CreateLandscapeController.class);
    ...
    /**
     * Controller method used to navigate from select participant screen to
     * create landscape screen
     * 
     */
    @RequestMapping(value = "/"
            + UIPathConstants.CREATELANDSCAPE_CREATELANDSCAPE, method = RequestMethod.POST)
    public void createLandscape(@ModelAttribute Participant participant,
            Model model) {
    ...
    /**
     * Controller method used to display create landscape wizard or landscape
     * settings screen based on landscape count
     * 
     */
    ...
    public String index(Model model, HttpServletRequest request) {
        ...
        if (Landscape.countLandscapes() == 0) {
            ...
            if (Participant.countParticipants() == 0) {
                ...
                model.addAttribute("enums", capabilities
                        .getParticipantSystemKinds().getSystemKind());
            ...
            //If there is single or more participant already exists show error message
                ...
                model.addAttribute(
                        "errormessage",
                        ctx.getMessage(ControllerConstants.CCF_MIS_CONFIGURATION_ERROR_MESSAGE));
        ...
        // Else show landscape settings screen
            ...
            if (createLandscapeHelper.verifyEntities(landscape, model, ctx)) {
                return UIPathConstants.CREATELANDSCAPE_DISPLAYERROR;
            } else if (createLandscapeHelper.verifyTFEntities(landscape, model,
                    ctx)) {
                return UIPathConstants.CREATELANDSCAPE_DISPLAYERROR;
            }
            //if all the required entities are available show landscape settings screen
                ...
                landscapeParticipantSettingsHelper
                        .populateParticipantSettingsModel(
    ...
    public void myInitBinder(WebDataBinder binder) {
        ...
        binder.setDisallowedFields(new String[] { "landscape.participant",
                "landscape.teamForge" });
    }
    ...
    public String saveLandscape(
            @RequestParam(ControllerConstants.PARTICIPANTHIDDEN) String participantparam,
            @ModelAttribute("landscapemodel") @Valid LandscapeModel landscapemodel,
        ...
        if (landscapemodel.getParticipant().getSystemKind()
        ...
        if (bindingResult.hasErrors()) {
            ...
            createLandscapeHelper.populateCreateLandscapeModel(model,
                    particpantHidden);
    ...
    public @ResponseBody
    String testTeamForgeConnection(@RequestParam("username") String username,
        ...
        try {
            ...
            returnText = "<strong><font color='green'>"
                    + ctx.getMessage(ControllerConstants.TF_CONNECTION_SUCCESS_MESSAGE)
                    + "</font></strong>";
        } catch (RemoteException remoteException) {
            log.debug(remoteException.getMessage());
            returnText = "<strong><font color='red'>"
                    + ctx.getMessage(ControllerConstants.TF_CONNECTION_FAILURE_MESSAGE)
                    + remoteException.getMessage() + "</font></strong>";
        }
        return returnText;
    }
