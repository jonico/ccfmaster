...
public class LandscapeFieldMappingTemplatesController extends AbstractLandscapeController {
    ...
    private static final Logger log                                        = LoggerFactory
                                                                                   .getLogger(LandscapeFieldMappingTemplatesController.class);
        ...
        try {
            ...
            for (FieldMappingLandscapeTemplate fieldMappingTemplate : fieldMappingLandscapeTemplateSession) {
                List<FieldMappingRule> newrules = FieldMappingUtil
                        .createFieldMappingRule(fieldMappingTemplate.getRules());
                List<FieldMappingValueMap> newValuemaps = FieldMappingUtil
                        .createFieldMappingValueMap(fieldMappingTemplate
                                .getValueMaps());
                for (String fmtName : items) {
                    ...
                    if (fieldMappingTemplate.getName().equals(fmtName)) {
                        if (isTemplateExists(nameFromMap, directions)) {
                            FieldMappingLandscapeTemplate fieldMappingLandscapeTemplatemerge = FieldMappingLandscapeTemplate
                                    .findFieldMappingLandscapeTemplatesByParentAndNameAndDirection(
                                            parent, nameFromMap, directions)
                                    .getSingleResult();
                            mergeFieldMappingTemplate(parent,
                                    fieldMappingTemplate,
                                    fieldMappingLandscapeTemplatemerge,
                                    newrules, newValuemaps, importStatus,
                                    nameFromMap, model, directions);
                        } else {
                            persistFieldMappingTemplate(parent, nameFromMap,
                                    fieldMappingTemplate, newrules,
    ...
    public String deleteFieldMappingTemplate(
            @RequestParam(ControllerConstants.DIRECTION) String paramdirection,
        ...
        if (paramdirection.equals(ControllerConstants.FORWARD)) {
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_DISPLAYFIELDMAPPINGTEMPLATESTFTOPART;
        } else {
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_DISPLAYFIELDMAPPINGTEMPLATESPARTTOTF;
        }
    }

    /**
     * Controller method to display field mapping templates for Participant to
     * TF
     * 
     * 
     */
    @RequestMapping(value = "/"
            + UIPathConstants.LANDSCAPESETTINGS_DISPLAYFIELDMAPPINGTEMPLATESPARTTOTF, method = RequestMethod.GET)
     ...
     */
    @RequestMapping(value = "/"
            + UIPathConstants.LANDSCAPESETTINGS_DISPLAYFIELDMAPPINGTEMPLATESTFTOPART, method = RequestMethod.GET)
    ...
    public void exportFieldMappingTemplate(
            @ModelAttribute("fieldMappingLandscapeTemplate") FieldMappingLandscapeTemplate fieldMappingLandscapeTemplateModel,
            @RequestParam(FMTID) String[] items, HttpServletResponse response) {
        ...
        try {
            ...
            XmlWebHelper.createJAXBExport(response, defaultFileName,
                    fieldMappingLandscapeTemplateList,
                    FieldMappingLandscapeTemplateList.class);
        } catch (Exception exception) {
            log.debug(
                    "Error exporting field mapping template: "
                            + exception.getMessage(), exception);
            FlashMap.setErrorMessage(
                    ControllerConstants.FMTEXPORTFAILUREMESSAGE,
                    exception.getMessage());
        }
    }
    ...
    public String importFieldMappingTemplate(
        ...
        try {
            ...
            populatePageSizetoModel(directions, model, session);
            return "redirect:/"
                    + UIPathConstants.LANDSCAPESETTINGS_UPLOADFIELDMAPPINGTEMPLATES
                    + "?direction=" + paramdirection;
        }
        try {
            List<FieldMappingLandscapeTemplate> fieldMappingLandscapeTemplatelist = fieldMappingLandscapeTemplateList
            ...
            for (FieldMappingLandscapeTemplate fieldMappingTemplate : fieldMappingLandscapeTemplatelist) {
                if (isTemplateExists(fieldMappingTemplate.getName(), directions)) {
                    fieldMappingTemplate
                            .setName(fieldMappingTemplate.getName());
            ...
            log.debug("Import failed due to following exception "
                    + exception.getMessage());
            FlashMap.setErrorMessage(
                    ControllerConstants.FMTIMPORTFAILUREMESSAGE,
                    exception.getMessage());
    ...
    public void myInitBinder(WebDataBinder binder) {
        ...
        binder.setDisallowedFields(new String[] {
                "fieldMappingLandscapeTemplate", "direction" });
    }
     ...
     * @param model
     */
    private void doList(Directions directions, Integer page, Integer size,
            Model model, HttpSession session) {
        Landscape landscape = ControllerHelper.findLandscape();
        ...
        List<FieldMappingLandscapeTemplate> fieldMappingLandscapeTemplate = paginate(
                ...
                FieldMappingLandscapeTemplate
                        .countFieldMappingLandscapeTemplatesByDirection(directions),
     ...
     * @return
     */
    private List<FieldMappingLandscapeTemplate> getFMTFromSession(
            HttpSession session) {
        FieldMappingLandscapeTemplateList fieldMappingLandscapeTemplateSessionList = (FieldMappingLandscapeTemplateList) session
                .getAttribute(FROMSESSION);
        List<FieldMappingLandscapeTemplate> fieldMappingLandscapeTemplateSession = fieldMappingLandscapeTemplateSessionList
                .getFieldMappingTemplate();
     ...
     */
    private void mergeFieldMappingTemplate(Landscape parent,
            FieldMappingLandscapeTemplate fieldMappingTemplate,
            FieldMappingLandscapeTemplate fieldMappingLandscapeTemplatemerge,
        ...
        try {
            fieldMappingLandscapeTemplatenew.setDirection(fieldMappingTemplate
                    .getDirection());
            fieldMappingLandscapeTemplatenew.setName(nameFromMap);
            fieldMappingLandscapeTemplatenew
                    .setId(fieldMappingLandscapeTemplatemerge.getId());
            fieldMappingLandscapeTemplatenew
                    .setVersion(fieldMappingLandscapeTemplatemerge.getVersion());
            ...
            fieldMappingLandscapeTemplatenew.setKind(fieldMappingTemplate
                    .getKind());
            fieldMappingLandscapeTemplatenew.merge();
            importStatus.put(fieldMappingTemplate.getName(),
                    UPDATED_SUCCESSFULLY);

        } catch (Exception exception) {
            log.debug(
                    "Error updating field mapping template: "
                            + exception.getMessage(), exception);
            if (isTemplateExists(fieldMappingTemplate.getName().toString(),
                    directions)) {
                importStatus.put(nameFromMap, UPDATED_FAILED
                        + FIELD_MAPPING_TEMPLATE_NAME_ALREADY_EXISTS);
            } else {
                importStatus.put(nameFromMap, UPDATED_FAILED);
            }
        }
     ...
     * @param fmtName
     */
    private void persistFieldMappingTemplate(Landscape parent,
            String nameFromMap,
            FieldMappingLandscapeTemplate fieldMappingTemplate,
            List<FieldMappingRule> newrules,
            List<FieldMappingValueMap> newValueMap,
        ...
        try {
            ...
            log.debug(
                    "Error saving field mapping template: "
                            + exception.getMessage(), exception);
     ...
     * @return
     */
    public static Map<String, String> mapIdandName(HttpServletRequest request,
            String[] items) {
        Map<String, String> nameMap = new HashMap<String, String>();
        String value = null;
        for (String fmtname : items) {
            if (request.getParameter("name-" + fmtname).equals("")
                    || request.getParameter("name-" + fmtname).equals(null)) {
                value = NULL_VALUE;
            } else {
    ...
    public static void populatePageSizetoModel(Directions directions,
        ...
        float nrOfPages = (float) FieldMappingLandscapeTemplate
                .countFieldMappingLandscapeTemplatesByDirection(directions)
                / size.intValue();
