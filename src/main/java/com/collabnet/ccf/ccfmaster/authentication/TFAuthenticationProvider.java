 ...
 * 
 */
public class TFAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {

    private static Logger log = LoggerFactory
                                      .getLogger(TFAuthenticationProvider.class);
    ...
    protected UserDetails retrieveUser(String username,
            UsernamePasswordAuthenticationToken token)
            throws AuthenticationException {
        Builder connectionBuilder = Connection.builder(serverUrl).proxy(proxy)
        ...
        try {
            if (username.startsWith("SESSIONID/")) {
                ...
                /*
                 * TODO: we're logging in with a pre-existing SOAP SessionID,
                 * but we pretend we're logging in via IAF and restricting
                 * access to the linkID (prplXXXX) supplied in the remainder of
                 * the username.
                 */
                 ...
                 * name. The IAF permissions will be added to the returned
                 * principal.
                 */
                String[] userParts = username.split("/", 2);
    ...
    private IafUserDetails createIafUserDetails(String username,
            Connection connection, String linkId) throws RemoteException {
        String projectPath = connection.getIntegratedAppClient()
                .getProjectPathByIntegratedAppId(linkId);
        return IafUserDetails.fromConnection(connection, username, "UNUSED",
                linkId, projectPath);
    }

    private TFUserDetails createTFUserDetails(String username,
            UsernamePasswordAuthenticationToken token, Connection connection,
        ...
        if (linkId != null) {
            ...
            return IafUserDetails.fromConnection(connection, username,
                    password, linkId, projectPath);
        } else {
            return TFUserDetails.fromConnection(connection, username, password,
                    token.getAuthorities());
        }
    }

    /*
     * public static class MapUserCache implements UserCache { private static
     * final Logger log = LoggerFactory.getLogger(MapUserCache.class); private
     * Map<String, UserDetails> userCache = new ConcurrentHashMap<String,
     * UserDetails>();
     * @Override public UserDetails getUserFromCache(String username) { final
     * UserDetails userDetails = userCache.get(username);
     * log.debug("retrieved user details for username {}: '{}'", username,
     * userDetails); return userDetails; }
     * @Override public void putUserInCache(UserDetails user) { final String
