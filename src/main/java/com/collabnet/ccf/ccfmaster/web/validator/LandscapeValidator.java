...
public class LandscapeValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        SystemKind participantkind = landscapeModel.getParticipant()
                .getSystemKind();
        if (participantkind.equals(SystemKind.QC)
                || participantkind.equals(SystemKind.SWP)) {
            if (Strings.isNullOrEmpty(landscapeModel
        ...
        if (Strings.isNullOrEmpty(landscapeModel.getTfUserNameLandscapeConfig()
                .getVal())) {
        ...
        if (participantkind.equals(SystemKind.QC)) {
            if (!landscapeModel.getParticipantUrlParticipantConfig().getVal()
                    .endsWith(ControllerConstants.VALIDATEQCURL)) {
                errors.rejectValue("participantUrlParticipantConfig.val",
                        ControllerConstants.ERROR_QCURL_VALIDATE);
            }
        }
        if (participantkind.equals(SystemKind.SWP)) {
            ...
            if (!landscapeModel.getParticipantUrlParticipantConfig().getVal()
                    .endsWith(ControllerConstants.VALIDATESWPURL)) {
                errors.rejectValue("participantUrlParticipantConfig.val",
                        ControllerConstants.ERROR_SWPURL_VALIDATE);
            }
        }
