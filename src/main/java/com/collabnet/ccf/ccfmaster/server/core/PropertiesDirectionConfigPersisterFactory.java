...
public class PropertiesDirectionConfigPersisterFactory implements
		DirectionConfigPersisterFactory {

	private static final Logger log = LoggerFactory.getLogger(PropertiesDirectionConfigPersisterFactory.class);
	...
	public PropertiesDirectionConfigPersisterFactory(File basePath) {
		...
		Assert.isTrue(
				basePath.exists() && basePath.isDirectory(),
	...
	public Persister<DirectionConfig> get(Direction direction) {
		...
		case REVERSE:
			...
			break;
		default:
			throw new IllegalArgumentException("unknown direction: " + direction.getDirection());
		}
		...
		fileName = String.format(baseName + ".properties",
				...
				landscape.getParticipant().getSystemKind())
				.toLowerCase();
		final File propFile = new File(propDir, fileName);
		log.debug("persisting to: %s", propFile);
		final PropertiesConfigItemPersister<DirectionConfig> persister = new PropertiesConfigItemPersister<DirectionConfig>(propFile);
