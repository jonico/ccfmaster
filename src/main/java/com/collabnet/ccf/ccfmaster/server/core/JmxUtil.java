...
final class JmxUtil {
		...
		/**
		 * Executes methodName on beanName with params/signatures in a separate thread.
		 * @see MBeanServerConnection#invoke(ObjectName, String, Object[], String[]) for parameter descriptions. 
		 * @param jmxPort
		 * @param beanName
		 * @param methodName
		 * @param params
		 ...
		 */
		public static Object executeMethod(final int jmxPort, final String beanName, final String methodName, final Object[] params, final String[] signatures) {
			/*
...
//						connection.getMBeanCount();
						...
						try {
							...
							// shouldn't happen.
							log.error("error closing JMX connection: " + e.getMessage(), e);
						}
					}
					return result;
				}
		...
		/**
		 * Connects to JMX on jmxPort and fetches the Memory attribute on "openadaptor:id=SystemUtil"
		 * @param jmxPort
		 * @return true if was able to get attribute, false if not successful or if timeout occurred
		 */
