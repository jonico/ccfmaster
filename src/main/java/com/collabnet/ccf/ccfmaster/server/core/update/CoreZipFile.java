...
public final class CoreZipFile implements Closeable, Serializable {
	...
	public void unzipTo(File destinationDir) throws IOException {
		...
		final CoreVersion landscapeVersion = CoreProperties.ofDirectory(destinationDir).getVersion();
		final CoreVersion myVersion = getVersion();
		if (myVersion.isNewerThan(landscapeVersion)) {
			log.info("unzipping {} (version {}) to {}", new Object[]{this.file, myVersion, destinationDir}); 
