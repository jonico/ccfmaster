...
public final class CoreZipFile implements Closeable, Serializable {
    ...
    /**
     * can be used once to setup this CoreZipFile instance if the no-arg
     * constructor was used.
     * 
     * @throws IllegalStateException
     *             if a file is set already
     */
    public void setFile(File file) throws ZipException, IOException {
        Preconditions.checkState(this.file == null, "Core file is already set",
                this.file);
        this.file = Preconditions.checkNotNull(file, "Core file can't be null");
        Preconditions.checkArgument(file.exists(), "Core file doesn't exist",
                file);
     ...
     * @throws IOException
     *             if unzipping fails
     * @throws CoreUpdateException
     *             if this core is not newer than the one at
     *             {@code destinationDir}
     */
    public void unzipTo(File destinationDir) throws IOException {
        ...
        Preconditions.checkArgument(
                ...
                "destination must be a directory that exists", destinationDir);
        Preconditions.checkState(this.file != null && this.file.exists(),
                "core zip file doesn't exist", this.file);
        Preconditions.checkState(this.validate(), "core zip file is invalid",
                this.file);
        final Version landscapeVersion = CoreProperties.ofDirectory(
                destinationDir).getVersion();
        final Version myVersion = getVersion();
        if (myVersion.isNewerThan(landscapeVersion)) {
            log.debug("unzipping {} (version {}) to {}", new Object[] {
                    this.file, myVersion, destinationDir });
            SingleLandscapeCCFCoreInteractionStrategy.unzip(this.file,
                    destinationDir);
        } else {
    ...
    public static CoreZipFile fromMultipartFile(MultipartFile upload)
    ...
    public static CoreZipFile fromServerInstance(File resourceFile)
            throws ZipException, IOException {
        File tempFile = null;
        try {
            tempFile = File.createTempFile("core", "zip");
