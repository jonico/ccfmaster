...
public class StartCoresOnBootBean {
    ...
    private static final Logger                     log                    = LoggerFactory
                                                                                   .getLogger(StartCoresOnBootBean.class);
    ...
    static final Function<Direction, CcfCoreStatus> direction2coreStatus   = new Function<Direction, CcfCoreStatus>() {
                                                                               @Override
                                                                               public CcfCoreStatus apply(
                                                                                       Direction input) {
                                                                                   return CcfCoreStatus
                                                                                           .findCcfCoreStatus(input
                                                                                                   .getId());
                                                                               }
                                                                           };
        ...
        for (Direction dir : allDirections) {
            if (dir.getShouldStartAutomatically() != null
                ...
                log.info(
                        ...
                        dir.getDescription(), dir.getDirection());
                bootCore(dir);
            }
        }
    }
    ...
    public List<Long> getRunningCoreIds() {
        ...
        if (runningCoreIds.isEmpty()) {
            final Iterable<CcfCoreStatus> runningCores = Iterables.filter(
                    getCores(), isRunning);
            runningCoreIds = ImmutableList.copyOf(Iterables.transform(
                    runningCores, core2Id));
        }
    ...
    private Iterable<CcfCoreStatus> getCores() {
        return Iterables.transform(Direction.findAllDirections(),
                direction2coreStatus);
    }
    ...
    CcfCoreStatus shutdownCore(Direction dir) {
        log.debug("About to shut down core for Direction ({}, {})...",
                dir.getDescription(), dir.getDirection());
        ...
        log.info("Sent stop command to core for Direction ({}, {}).",
                dir.getDescription(), dir.getDirection());
        return shutDownCore;
    }
