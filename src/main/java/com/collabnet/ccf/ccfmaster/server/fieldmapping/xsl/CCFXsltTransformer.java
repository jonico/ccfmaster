...
public class CCFXsltTransformer {
    ...
    private void setGenericArtifactToRepositoryTransformer(String dir) {
        String filePath = String.format("%s%s%s", dir, File.separator,
                GENERIC_ARTIFACT_TO_REPOSITORY_XSLT_FILE);
        this.genericArtifactToRepositoryTransformer = processXslResource(new ClassPathResource(
                filePath));
    }

    private void setRepositoryToGenericArtifactTransformer(String dir) {
        String filePath = String.format("%s%s%s", dir, File.separator,
                REPOSITORY_TO_GENERIC_ARTIFACT_XSLT_FILE);
        this.repositoryToGenericArtifactTransformer = processXslResource(new ClassPathResource(
                filePath));
    }

    private Document transform(Transformer transformer, Document d)
            throws TransformerException {
