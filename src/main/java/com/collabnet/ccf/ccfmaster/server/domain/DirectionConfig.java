...
@RooJavaBean
@RooToString
@RooEntity(finders = { "findDirectionConfigsByDirection", "findDirectionConfigsByDirectionAndName" })
@Table(uniqueConstraints = @UniqueConstraint(columnNames = {"direction", "name"}))
@XmlRootElement
...
public class DirectionConfig implements ConfigItem, PersistableConfigItem<DirectionConfig> {
    ...
    public static long countDirectionConfigsByDirection(Direction direction) {
        ...
        TypedQuery<Long> q = em.createQuery("SELECT COUNT(directionconfig) FROM DirectionConfig AS directionconfig WHERE directionconfig.direction = :direction", Long.class);
