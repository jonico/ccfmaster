...
public class XsltValidator implements ConstraintValidator<SafeXslt, Element> {
    ...
    private static final class ValidatingErrorListener implements ErrorListener {
        ...
        private static Function<String, String>  messageForFunctionName = new Function<String, String>() {
                                                                            ...
                                                                            public String apply(
                                                                                    String input) {
                                                                                return String
                                                                                        .format("The first argument to the non-static Java function '%s' is not a valid object reference.",
                                                                                                input);
                                                                            }
                                                                        };

        public ValidatingErrorListener(Iterable<String> allowedFunctions,
                ConstraintValidatorContext context) {
            this.context = context;
            this.allowedErrorMessages = Iterables.transform(allowedFunctions,
                    messageForFunctionName);
        }
        ...
        private boolean expectedError(TransformerException exception) {
            return Iterables.contains(allowedErrorMessages,
    ...
    public void initialize(SafeXslt constraintAnnotation) {
        allowedFunctions = ImmutableSet.copyOf(constraintAnnotation
    ...
    public boolean isValid(Element xslt, ConstraintValidatorContext context) {
        ...
        try {
            ...
            final ValidatingErrorListener listener = new ValidatingErrorListener(
                    allowedFunctions, context);
            ...
            /*
             * Transformer transformer = factory.newTransformer(source); if
             * (!listener.getErrorOccurred()) { final ValidatingErrorListener
             * listener2 = new ValidatingErrorListener(allowedFunctions,
             * context); transformer.setErrorListener(listener2); StreamSource
             * ss = new StreamSource(new StringReader(
             * "<foo xmlns:ccf='http://ccf.open.collab.net/GenericArtifactV1.0'><ccf:field fieldName='description'>asdf</ccf:field><ccf:field fieldName='title'>asdf</ccf:field></foo>"
             * )); transformer.transform(ss, new XMLResult()); result =
             * !listener2.getErrorOccurred(); }
             */
        } catch (Exception e) {
    ...
    private static XPath buildXpath(Element xslt, final String functionName) {
        XPath xpath = xslt
                .createXPath(String
                        .format("//xsl:value-of[starts-with(@select, 'stringutil:%s(string(.))')]",
                                functionName));
