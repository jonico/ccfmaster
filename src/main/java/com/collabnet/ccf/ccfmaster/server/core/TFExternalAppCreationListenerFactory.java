...
public class TFExternalAppCreationListenerFactory implements ExternalAppCreationListenerFactory {
    ...
    static class TFExternalAppCreationListener implements ExternalAppCreationListener {
        private static final Logger               log = LoggerFactory
                                                              .getLogger(TFExternalAppCreationListener.class);
        private final IntegratedApplicationClient client;
        private final TeamForgeClient             tfc;
        private final String                      baseUrl;

        public TFExternalAppCreationListener(String baseUrl,
                IntegratedApplicationClient client,
                TeamForgeClient teamForgeClient) {
            ...
            try {
                ...
                if (linkId == null) {
                    ...
                    final String prefix = CreateIntegratedAppStrategy.prefix
                            + landscape.getId();
                    NamedValues ret = client.enablePluggableComponent(
                            project.getId(), plugId,
                            new PluggableComponentParameterDO[0], prefix);
                    if (log.isDebugEnabled()) {
                        final String[] names = ret.getNames();
                        final String[] values = ret.getValues();
                        log.debug(
                                "result of enabling IA for project {} ({} names, {} values):",
                                new Object[] { projectPath, names.length,
                                        values.length });
         ...
         * 
         * @param projectPath
         * @return if the IA is associated to projectPath, returns the linkId,
         *         null otherwise.
        ...
        private String getLinkId(String projectPath) throws RemoteException {
            try {
                ...
                if (e.getMessage()
                        .matches(
                                "No such object: Integrated application 'plug\\d+' is not associated to project 'proj\\d+'")) {
    ...
    public TFExternalAppCreationListener get() {
        ...
        try {
            final Connection connection = ((TFUserDetails) user)
                    .getConnection();
            return new TFExternalAppCreationListener(baseUrl,
                    connection.getIntegratedAppClient(),
