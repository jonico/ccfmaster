...
@Table(uniqueConstraints = @UniqueConstraint(columnNames = { "LANDSCAPE",
        "DIRECTION" }))
@RooEntity(finders = { "findDirectionsByLandscapeEquals",
...
public class Direction {
    ...
    public File determineBaseDirectory(File ccfHome) {
        ...
        File res = new File(ccfHome, String.format("landscape%d/samples/%s/%s",
                landscape.getId(), scenario, baseName));
        Assert.isTrue(res.exists(), "Scenario base directory doesn't exist: "
                + res);
        return res;
    }
    ...
    private String baseName() {
        ...
        if (participantSystemKind.equals(SystemKind.GENERIC)) {
            return String.format(
                    getDirection() == Directions.FORWARD ? "%1$s2%2$s"
                            : "%2$s2%1$s", teamForgeSystemKind, landscape
                            .getParticipant().getPrefix());
        }
        return String.format(getDirection() == Directions.FORWARD ? "%1$s2%2$s"
                : "%2$s2%1$s", teamForgeSystemKind, participantSystemKind);
    }
    ...
    public static long countDirectionsByDirection(Directions direction) {
        ...
        TypedQuery<Long> q = em
                .createQuery(
                        "SELECT COUNT(direction) FROM Direction AS direction WHERE direction.direction = :direction",
                        Long.class);
    ...
    public static long countDirectionsByLandscapeAndDirection(
        ...
        TypedQuery<Long> q = em
                .createQuery(
                        "SELECT COUNT(direction) FROM Direction AS direction WHERE direction.landscape = :landscape  AND direction.direction = :direction",
                        Long.class);
    ...
    public static long countDirectionsByLandscapeEquals(Landscape landscape) {
        ...
        TypedQuery<Long> q = em
                .createQuery(
                        "SELECT COUNT(direction) FROM Direction AS direction WHERE direction.landscape = :landscape",
                        Long.class);
        q.setParameter("landscape", landscape);
        return q.getSingleResult();
    }
