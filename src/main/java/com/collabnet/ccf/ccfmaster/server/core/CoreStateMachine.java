...
/**
 * this class implements part of the state diagram at {@link http
 * ://oryx-project.
 * org/oryx/editor;petrinet?stencilset=/stencilsets/petrinets/petrinet
 * .json#/model/13150/}
 * 
 * Some of the state transitions are performed by separate threads that monitor
 * launchin/stopping cores.
 * 
 * @author ctaylor
 * 
 */
...
public class CoreStateMachine {
    public static class CoreMonitor {
        ...
        private CoreMonitor(int jmxPort, CcfCoreStatus statusToUpdate,
            ...
            return new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        for (long i = 0; i < pollCount; i++, delayUnit
                                .sleep(delayUnitCount)) {
                            if (JmxUtil.canConnect(jmxPort).equals(
                                    JmxStatus.CONNECTED)) {
                                state = STARTED;
                                break;
                            }
            ...
            return new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        for (int i = 0; i < pollCount; i++, delayUnit
                                .sleep(delayUnitCount)) {
                            if (!JmxUtil.canConnect(jmxPort).equals(
                                    JmxStatus.CONNECTED)) {
                                state = STOPPED;
                                command = (statusToUpdate.executedCommand() == RESTART) ? RESTART
                                        : command;
                                break;
                            }
                        }
        ...
        public static CoreMonitor builder(int jmxPort,
                CcfCoreStatus statusToUpdate, CoreStateMachine stateMachine) {
            ...
            if (proc != null) {
                ...
                try {
                    ...
                    throw new CoreConfigurationException(
                            "interrupted while waiting for core to die: "
                                    + e.getMessage(), e);
                }
            }
        }
            ...
            try {
                ...
                switch (command) {
                    case START:
                        ...
                        final Runnable startingMonitor = CoreMonitor.builder(
                                determineJmxPort(status), status, stateMachine)
                                .startingMonitor();
                    ...
                    case STOP:
                        ...
                        final Runnable stoppingMonitor = CoreMonitor
                                .builder(determineJmxPort(status), status,
                                        stateMachine).connectionAttempts(45)
                                .stoppingMonitor();
                ...
                throw new CoreConfigurationException("error starting core: "
                        + e.getMessage(), e);
            }
        }
        ...
        void handleTransition(CcfCoreStatus status, ExecutedCommand command) {
            ...
            if (state == CoreState.STOPPED
                ...
                try {
                    ...
                    status.setExecutedCommand(command);
                    status.coreProcess(proc);
                    Runnable monitor = CoreMonitor.builder(
                            determineJmxPort(status), status, stateMachine)
                            .startingMonitor();
                    executor.execute(monitor);
                } catch (IOException e) {
                    throw new CoreConfigurationException(
                            "error starting core: " + e.getMessage(), e);
                }
            }
        }
        ...
        void handleTransition(CcfCoreStatus status, ExecutedCommand command) {
            ...
            if (state == STARTED && (command == STOP || command == RESTART)) {
                ...
                Runnable monitor = CoreMonitor
                        ...
                        .connectionAttempts(45).stoppingMonitor();
                executor.execute(monitor);
            }
        }
    ...
    public static abstract class TransitionObserver {
        ...
        private static final Logger log = LoggerFactory
                                                .getLogger(TransitionObserver.class);
        protected CoreStateMachine  stateMachine;

        public void onUpdate(CoreStateMachine stateMachine,
                CcfCoreStatus statusToUpdate, ExecutedCommand command) {
            ...
            if (newState != oldState || newCommand != oldCommand) {
                ...
                log.debug("{}: Transition: ({},{}) -{}-> ({},{})",
                        new Object[] { getClass().getSimpleName(), oldState,
                                oldCommand, command, newState, newCommand });
            } else {
                //				log.debug("{}: No change on command {}: ({},{})", new Object[]{getClass().getSimpleName(), command, oldState, oldCommand});
            }
        }

        abstract void handleTransition(CcfCoreStatus statusToUpdate,
    ...
    static abstract class ProcessTransitionObserver extends TransitionObserver {
        ...
        protected File determineBaseDir(Direction direction) {
            ...
            final SystemKind teamForgeSystemKind = landscape.getTeamForge()
                    .getSystemKind();
            final SystemKind participantSystemKind = landscape.getParticipant()
                    .getSystemKind();
            final Participant participant = landscape.getParticipant();
            Assert.isTrue(teamForgeSystemKind == SystemKind.TF,
                    "landscape.teamForge must be of Kind TF, was: "
                            + teamForgeSystemKind);
            ...
            switch (participantSystemKind) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "CCF doesn't support mapping TF <-> "
                                    + participantSystemKind);
            }
            String baseName = directionBaseName(direction);
            File res = new File(getCcfHome(), String.format(
                    "landscape%d/samples/%s/%s", landscape.getId(), scenario,
                    baseName));
            Assert.isTrue(res.exists(),
                    "Scenario base directory doesn't exist: " + res);
            return res;
        }
        ...
        protected int determineJmxPort(CcfCoreStatus status) {
            ...
            return direction == Directions.REVERSE ? jmxReversePort
                    : jmxForwardPort;
        }

        protected Process startCore(final CcfCoreStatus status)
                throws IOException {
            ...
            final Process proc = builder.command(script.getPath())
        ...
        protected void stopCore(CcfCoreStatus status) {
            ...
            JmxUtil.executeMethod(jmxPort, "openadaptor:id=ServiceWrapperBean",
                    "stop", new Object[] { 0 }, new String[] { "int" });
        }
        ...
        private static String directionBaseName(Direction direction) {
            ...
            if (participantSystemKind.equals(SystemKind.GENERIC)) {
                return String
                        .format(direction.getDirection() == Directions.FORWARD ? "%1$s2%2$s"
                                : "%2$s2%1$s", teamForgeSystemKind, landscape
                                .getParticipant().getPrefix());
            }
            return String
                    .format(direction.getDirection() == Directions.FORWARD ? "%1$s2%2$s"
                            : "%2$s2%1$s", teamForgeSystemKind,
                            participantSystemKind);
        }
    }

    private static final Logger      log       = LoggerFactory
                                                       .getLogger(CoreStateMachine.class);
    ...
    public CoreState determineCurrentStatus(CcfCoreStatus status,
            long toleratedPollingInterval) {
    ...
    public void notifyObservers(final CcfCoreStatus status,
            final ExecutedCommand newCommand) {
    ...
    private boolean canConnectToPort(int jmxPort) {
        ...
        try {
            ...
            throw new IllegalStateException("couldn't resolve localhost: "
                    + e.getMessage(), e);
        } catch (IOException e) {
            return false;
        } finally {
