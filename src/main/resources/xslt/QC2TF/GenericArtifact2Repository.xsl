...
<xsl:stylesheet version="2.0"
	...
	<xsl:template match="/node()" priority="2">
		<xslo:stylesheet version="2.0"
			xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
			xmlns:ccf="http://ccf.open.collab.net/GenericArtifactV1.0" xmlns:fn="http://www.w3.org/2005/xpath-functions"
			xmlns="" xmlns:stringutil="xalan://com.collabnet.ccf.core.utils.GATransformerUtil" exclude-result-prefixes="xsl xs fn ccf stringutil">
			<xsl:comment>
			...
			<xslo:template match="/ccf:artifact" priority="2">
				<xsl:element name="{local-name()}">
					<xsl:element name="topLevelAttributes">
						<xslo:copy-of
							select="@* [not(name()='xmlns:xsi' or name() ='xsi:schemaLocation')] " />
					</xsl:element>
	...
	<xsl:template match="ccf:field" priority="2">
		<xslo:template
			match="ccf:field[@fieldName='{@fieldName}' and @fieldType='{@fieldType}']"
			priority="2">
			<xsl:element name="{@alternativeFieldName}">
				<!--
					<xslo:copy-of select="@fieldValueHasChanged"/> <xslo:copy-of
					select="@fieldAction"/>
				-->
				<xsl:choose>
					<xsl:when test="@fieldValueType='HTMLString'">
						<xslo:value-of select="stringutil:stripHTML(string(.))" />
					</xsl:when>
					<xsl:otherwise>
