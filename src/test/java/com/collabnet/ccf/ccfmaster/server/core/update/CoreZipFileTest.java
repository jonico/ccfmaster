...
public class CoreZipFileTest {
	...
	public void unzipDoesntOverwriteNewerLandscape() throws IOException {
		writeCorePropFile(propFile, new CoreVersion(200,0,0,""), "foo");
	...
	public void unzipCreatesNewFiles() throws IOException {
		writeCorePropFile(propFile, new CoreVersion(0,0,0,""), "foo");
		CoreZipFile czf = null;
		boolean threw = true;
		try {
			czf = createCoreZipFile(VALID_ZIP_FILE_NAME);
	...
	public void unzipBadZipFileThrowsZipFileException() throws ZipException, IOException {
		CoreZipFile czf = null;
		try {
	...
	@Test
	public void unzipInvalidZipFileDoesNotValidate() throws ZipException, IOException {
		CoreZipFile czf = null;
		boolean threw = true;
		try {
	...
	CoreZipFile createCoreZipFile(String fileName) throws IOException, ZipException {
		URL testCoreZip = Resources.getResource(CoreZipFileTest.class, fileName);
		MultipartFile upload = new MockMultipartFile("file", Resources.newInputStreamSupplier(testCoreZip).getInput());
		return CoreZipFile.fromMultipartFile(upload);
	}

	

	private void writeCorePropFile(File propFile, CoreVersion version, String desc) throws FileNotFoundException,
			IOException {
		Properties props = new Properties();
		props.setProperty(CoreVersion.CCFCORE_MAJOR_VERSION, Integer.toString(version.getMajor()));
		props.setProperty(CoreVersion.CCFCORE_MINOR_VERSION, Integer.toString(version.getMinor()));
		props.setProperty(CoreVersion.CCFCORE_PATCH_VERSION, Integer.toString(version.getPatch()));
		props.setProperty(CoreVersion.CCFCORE_REVISION_STRING, version.getRevision());
