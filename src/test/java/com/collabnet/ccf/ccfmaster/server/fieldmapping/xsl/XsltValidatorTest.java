...
public class XsltValidatorTest {
    ...
    public void functionCallsExistInSampleResources() throws Exception {
        for (String resourceName : ImmutableList.of(
                "MapForceToGenericArtifactFormat.xsl", "custom.xsl")) {
            URL xsltLocation = Resources.getResource(XsltValidatorTest.class,
                    resourceName);
            final String xslt = Resources
                    .toString(xsltLocation, Charsets.UTF_8);
    ...
    public void functionCallsFound() throws DocumentException {
        final ImmutableList<String> data = ImmutableList
                .of("<xsl:value-of xmlns:xsl='http://www.w3.org/1999/XSL/Transform' select='stringutil:foo(string(.))'/>",
        ...
        for (String xml : data) {
            final Element rootElement = DocumentHelper.parseText(xml)
                    .getRootElement();
            assertEquals(1, XsltValidator.findFunctionCalls(rootElement, "foo")
                    .size());
        }
    }
    ...
    public void noFunctionCallsFound() throws DocumentException {
        final ImmutableList<String> data = ImmutableList
                .of("<xsl:value-of xmlns:xsl='http://www.w3.org/1999/XSL/Transform' select='stringutil:foo(.)'/>",
        ...
        for (String xml : data) {
            final Element rootElement = DocumentHelper.parseText(xml)
                    .getRootElement();
            assertEquals(0, XsltValidator.findFunctionCalls(rootElement, "foo")
                    .size());
        }
    }
}
