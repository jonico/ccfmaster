...
public class StorageDirectoryTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	@Autowired private FieldMappingExternalAppTemplateDataOnDemand fmeatDod;
	@Autowired private FieldMappingLandscapeTemplateDataOnDemand fmltDod;
	
	private File baseDir = null;
	
	@Before
	public void createTempDir() throws IOException {
		baseDir = File.createTempFile(StorageDirectoryTest.class.getName(), "dir");
		baseDir.delete();
		baseDir.mkdir();
	}
	@Test
	public void testFieldMappingDirectory() {
		...
		assertTrue("RMD direction not in directory", dir.toString().contains(rmd.getDirection().toString()));
	}
	
	@Test
	public void testFieldMappingExternalAppTemplateDirectory() {
		final FieldMappingExternalAppTemplate fmeat = fmeatDod.getNewTransientFieldMappingExternalAppTemplate(0);
		final File dir = mappingStorageDirectoryTests(fmeat);
		assertTrue("template direction not in directory " + dir, dir.toString().toLowerCase().contains(fmeat.getDirection().toString().toLowerCase()));
		assertTrue("linkId not in directory " + dir, dir.toString().toLowerCase().matches(".*?prpl\\d+.*"));
	}
	
	@Test
	public void testFieldMappingLandscapeTemplateDirectory() {
		final FieldMappingLandscapeTemplate fmeat = fmltDod.getNewTransientFieldMappingLandscapeTemplate(0);
		final File dir = mappingStorageDirectoryTests(fmeat);
		assertTrue("template direction not in directory " + dir, dir.toString().toLowerCase().contains(fmeat.getDirection().toString().toLowerCase()));
	}
...
//		assertTrue(dir + " is not a directory " + dir, dir.isDirectory());
		assertTrue("landscape not in directory path " + dir, dir.toString().toLowerCase().matches(".*?landscape\\d+.*"));
		return dir;
	}
