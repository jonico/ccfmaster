 ...
 * 
 */
public class TempfilePropertiesDirectionConfigPersisterFactory implements
	...
	public Persister<DirectionConfig> get(Direction direction) {
		try {
			File newFile = File.createTempFile("test-" + direction.getId(), ".properties");
			newFile.deleteOnExit();
			File cacheFile = fileCache.putIfAbsent(direction, newFile);
			if (cacheFile == null) {
				return new PropertiesConfigItemPersister<DirectionConfig>(newFile);
			} else {
				// a file for this direction was in cache already, use it and
				// delete the unused replacement.
				newFile.delete();
				return new PropertiesConfigItemPersister<DirectionConfig>(cacheFile);
			}
		} catch (IOException e) {
