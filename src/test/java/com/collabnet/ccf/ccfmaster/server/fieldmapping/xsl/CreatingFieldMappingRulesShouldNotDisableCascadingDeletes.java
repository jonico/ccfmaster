...
public class CreatingFieldMappingRulesShouldNotDisableCascadingDeletes extends AbstractTransactionalJUnit4SpringContextTests {
    ...
    public void deleteInactiveFieldMapping() {
        assertNotNull(fm);
        final RepositoryMappingDirection parent = fm.getParent();
        assertTrue("mapping is active", parent.getActiveFieldMapping() == null
                || parent.getActiveFieldMapping().getId().equals(fm.getId()));
        long id = fm.getId();
        fm.remove();
        fm.flush();
        assertNull(FieldMapping.findFieldMapping(id));
