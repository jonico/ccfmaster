...
public class ProjectScopeIdMappingRoleLogin extends ProjectScopeTestBase{
	...
	public ProjectScopeIdMappingRoleLogin() {
		super();
		LoginInfo user = LoginInfo.projectIdentityMappingScopeFromSystemProperties();
		user.login(selenium);
	}
	...
	public void checkIdMappingRoles() {
		...
		assertThat(roles, hasItems("ROLE_IDENTITY_MAPPINGS","ROLE_IAF_USER","ROLE_TF_USER"));
	}
	...
	public void checkFieldMappingRoles() {
		IndexPage index = new IndexPage(driver);
		final List<String> roles = index.getRoles();
		assertThat(roles, hasItems("ROLE_MAPPING_RULE_TEMPLATES", "ROLE_IAF_USER","ROLE_TF_USER"));
	}
