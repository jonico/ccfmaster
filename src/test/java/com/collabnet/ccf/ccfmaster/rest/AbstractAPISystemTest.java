...
public abstract class AbstractAPISystemTest {
	...
	public void createTestData() {
		...
		tf = restTemplate.postForObject(ccfAPIUrl + "/participants", tf, Participant.class);
		Map<String, String> vars = Collections.singletonMap("id", tf.getId()+"");
		//Headers headers = new Headers();
		
		tf.setTimezone(Timezone.Europe_SLASH_Paris);
		restTemplate.put(ccfAPIUrl + "/participants/{id}", new HttpEntity<Participant> (tf, headers), vars);
		...
		qc = restTemplate.postForObject(ccfAPIUrl + "/participants", qc, Participant.class);
		
		qcconfig = new ParticipantConfig();
		qcconfig.setParticipant(qc);
		qcconfig.setName("participant.foo");
		qcconfig.setVal("bar");
		qcconfig = restTemplate.postForObject(ccfAPIUrl + "/participantconfigs", qcconfig, ParticipantConfig.class);
		...
		qctf = restTemplate.postForObject(ccfAPIUrl + "/landscapes", new HttpEntity<Landscape> (qctf, headers), Landscape.class);
		
		qctfconfig = new LandscapeConfig();
		qctfconfig.setLandscape(qctf);
		qctfconfig.setName("landscape.foo");
		qctfconfig.setVal("bar");
		qctfconfig = restTemplate.postForObject(ccfAPIUrl + "/landscapeconfigs", qctfconfig, LandscapeConfig.class);
		
		qc2tf = new Direction();
		qc2tf.setDescription("QC-TF");
		qc2tf.setLandscape(qctf);
		qc2tf.setDirection(Directions.REVERSE);
		qc2tf = restTemplate.postForObject(ccfAPIUrl + "/directions", qc2tf, Direction.class);
		
		qc2tfconfig = new DirectionConfig(); 
		qc2tfconfig.setDirection(qc2tf);
		qc2tfconfig.setName("direction.foo");
		qc2tfconfig.setVal("bar");
		qc2tfconfig = restTemplate.postForObject(ccfAPIUrl + "/directionconfigs", qc2tfconfig, DirectionConfig.class);
		
		tf2qc = new Direction();
		tf2qc.setDescription("TF-QC");
		tf2qc.setLandscape(qctf);
		tf2qc.setDirection(Directions.FORWARD);
		tf2qc = restTemplate.postForObject(ccfAPIUrl + "/directions", tf2qc, Direction.class);
		
		ea = new ExternalApp();
		ea.setLandscape(qctf);
		ea.setLinkId("prpl1043");
		ea.setProjectPath("projects.brokerage_system_sample");
		ea = restTemplate.postForObject(ccfAPIUrl + "/externalapps", ea, ExternalApp.class);
		...
		rm = restTemplate.postForObject(ccfAPIUrl + "/repositorymappings", rm, RepositoryMapping.class);
		...
		rmd = restTemplate.postForObject(ccfAPIUrl + "/repositorymappingdirections", rmd, RepositoryMappingDirection.class);
	}
	
	@After
	public void wipeTestData() {
		LandscapeList landscapes = restTemplate.getForObject(ccfAPIUrl + "/landscapes", LandscapeList.class);
		...
		for (Participant participant : participants) {
			restTemplate.delete(ccfAPIUrl + "/participants/" + participant.getId());
		}
	}
