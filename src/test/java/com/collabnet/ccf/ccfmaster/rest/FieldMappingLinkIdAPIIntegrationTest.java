...
public class FieldMappingLinkIdAPIIntegrationTest extends AbstractAPIIntegrationTest {
    ...
    public void testFind() {
        ...
        String linkIdPathSegment = "/linkid/" + obj.getParent().getRepositoryMapping().getExternalApp().getLinkId() + "/fieldmappings/";  
        obj = restTemplate.getForObject(ccfAPIUrl + linkIdPathSegment +  id, FieldMapping.class);
    ...
    public void testCount() {
    	FieldMapping obj = dod.getRandomFieldMapping();
    	org.junit.Assert.assertNotNull("Data on demand for 'FieldMapping' failed to initialize correctly", obj);
        long count = FieldMapping.countFieldMappingsByExternalApp(obj.getParent().getRepositoryMapping().getExternalApp());
        String linkIdPathSegment = "/linkid/" + obj.getParent().getRepositoryMapping().getExternalApp().getLinkId() + "/fieldmappings/";
        List<FieldMapping> result = restTemplate.getForObject(ccfAPIUrl + linkIdPathSegment, FieldMappingList.class);
    ...
    public void testCountRepositoryMappingDirectionScope() {
        ...
        String linkIdPathSegment = "/linkid/" + obj.getParent().getRepositoryMapping().getExternalApp().getLinkId() + "/repositorymappingdirections/" + obj.getParent().getId() + "/fieldmappings/";
        List<FieldMapping> result = restTemplate.getForObject(ccfAPIUrl + linkIdPathSegment, FieldMappingList.class);
        org.junit.Assert.assertTrue("Counter for 'FieldMapping' incorrectly reported there were no entries", count > 0);
        org.junit.Assert.assertNotNull("Find entries method for 'FieldMapping' illegally returned null", result);
        org.junit.Assert.assertEquals("Find entries method for 'FieldMapping' returned an incorrect number of entries", count, result.size());
    ...
    public void testCreate() {
    	RepositoryMappingDirection randomObject = dodRM.getRandomRepositoryMappingDirection();
    	org.junit.Assert.assertNotNull("Data on demand for 'FieldMapping' failed to initialize correctly", randomObject);
    	String linkIdPathSegment = "/linkid/" + randomObject.getRepositoryMapping().getExternalApp().getLinkId() + "/fieldmappings/";
        ...
        obj = restTemplate.postForObject(ccfAPIUrl + linkIdPathSegment, obj, FieldMapping.class);
