...
public class LogFileAPIIntegrationTest extends AbstractAPIIntegrationTest{
	...
	@Before
	public void setUp(){
		String logDirString = String.format("%s%d%s", "landscape",dod.getSpecificDirection(0).getLandscape().getId(),"/samples/TFSWP/TF2SWP/logs");
		File logDir = new File(ccfHome, logDirString);
		logDir.mkdirs();
		try {
			Files.touch(new File(logDir, "sample-ccf-info.log"));
			FileUtils.write(new File(logDir, "sample-ccf-info.log"), "Testing\n logFile\n Let see who testing goes\n");
		} catch (IOException e) {	}
	}
	
	@After
	public void clear(){
		String deleteDirString = String.format("%s%d", "landscape",dod.getSpecificDirection(0).getLandscape().getId());
    ...
    public void testLogFileListFind() throws IOException {
        ...
        LogFileList logObj = restTemplate.getForObject(ccfAPIUrl + "/directions/"+ id +"/logs/", LogFileList.class);
        org.junit.Assert.assertNotNull("Find method for 'Direction' illegally returned null for id '" + id + "'", logObj);
    }
	
    /*
     * Below method is commented because rest template expects header values to be set 
     * Since header is missing below test case throws exception stating content-type is not set.
     *@Test
    public void testLogFile() throws IOException, URISyntaxException {
        Direction obj = dod.getSpecificDirection(1);
        org.junit.Assert.assertNotNull("Data on demand for 'Direction' failed to initialize correctly", obj);
        java.lang.Long id = obj.getId();
        org.junit.Assert.assertNotNull("Data on demand for 'Direction' failed to provide an identifier", id);
        String value = restTemplate.getForObject(ccfAPIUrl + "/directions/"+ id +"/logs/sample-ccf-info.log?firstLine=1&maxlines=2",String.class);
    }*/
