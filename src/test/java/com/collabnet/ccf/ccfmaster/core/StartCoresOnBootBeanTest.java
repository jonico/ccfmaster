...
public class StartCoresOnBootBeanTest {
	...
	@Mocked(methods={"merge()", "findCcfCoreStatus(Long)", "getCurrentStatus()"}) CcfCoreStatus ccs;
	@Mocked(methods={"findAllDirections()"}) Direction dir;
	...
	public void bootCallsMergeIfShouldStartAutomatically() {
		...
		ccs.setId(id);
		new Expectations() {{
				Direction.findAllDirections(); returns(Arrays.asList(dir));
				CcfCoreStatus.findCcfCoreStatus(id); returns(ccs);
				ccs.merge();
		}};
	...
	public void shutdownCallsMergeAndThenCallsGetCurrentStatusUntilStopped() {
		...
		new Expectations() {{
				Direction.findAllDirections(); returns(Arrays.asList(dir));
				CcfCoreStatus.findCcfCoreStatus(id); returns(ccs);
				ccs.merge(); returns(ccs);
				ccs.getCurrentStatus(); returns(Arrays.asList(CoreState.STOPPING, CoreState.STOPPING, CoreState.STOPPED));
		}};
	...
	public void shutdownCallsMergeAndThenCallsGetCurrentStatusUntilPollCountReached() {
		...
		new Expectations() {{
				...
				CcfCoreStatus.findCcfCoreStatus(id); returns(ccs);
				ccs.merge(); returns(ccs);
				ccs.getCurrentStatus(); returns(Arrays.asList(CoreState.STOPPING, CoreState.STOPPING));
		}};
		final StartCoresOnBootBean scobb = new StartCoresOnBootBean();
